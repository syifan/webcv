# EasyCV React Local Example

Same as `example/react`, but `easycv` is wired to the local package via `file:../..` so you can test unpublished changes.

## Local dev

```bash
cd example/react-local
...
npm run build              # outputs to example/react-local/dist
```
